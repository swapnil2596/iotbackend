pipeline {
  ...
  stages {
    stage("Git Staging") {
        steps{
                checkout([$class: 'Git', branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: 'ubuntu', url: 'https://github.com/swapnil2596/iotbackend.git']]])
            }
    }
    ...
    stage('Build the Image') {
        steps{
            ...
            sh 'docker image rmi -f $imagename:v1.$BUILD_ID swapnil257/$imagename:v1.$BUILD_ID swapnil257/$imagename:latest'
        }
    }
